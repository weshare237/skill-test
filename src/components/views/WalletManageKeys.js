...
function WalletManageKeys(props) {
  ...
  const confirmPassword = (password) => {
    ...
    axios
      .post(SERVER_URL + 'wallets/getsecret', {
      ...
      <Col className='p-4' span={24}>
        <Row className='mt-4 myColor1 '>
          <Col span={12} className=' font-bold'>
            <AiOutlineQrcode size={20} className='inline mr-2' />
            {t('Wallet Address')}
          </Col>
        ...
        <Row className='mt-4 myColor1'>
          ...
          <Col span={6} className='text-right'>
            <button onClick={setContentPhrase}>
              <AiTwotoneContainer size={20} className='inline mr-2' />
            </button>
            {/* Duclair: Anchor used as a button. Anchors are primarily expected to navigate. Use the button element instead. */}
          </Col>
          <Col span={6} className='text-right'>
            <button onClick={setContentPrivateKey}>
