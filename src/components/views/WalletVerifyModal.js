...
function WalletVerifyModal(props) {
  ...
  return (
    <WalletModal
      title={t('Verify Mnemonic')}
      close={() => props.setModalShow(false)}
    >
      <p className='text-lg font-bold myColor1 mt-8'>
        {t('Fill in Mnemonic Phrase Below')}
      </p>
      <Row>
        {props.typedMnemonic.map((item, idx) =>
          props.missPhrase.includes(idx) ? (
            <div
              className='p-2 mt-2 myColor1 font-bold  text-lg w-1/3'
              key={idx} // Duclair: Do not use Array index in keys
            >
              <div className='border-b-2 border-gray-500 flex'>
                ...
                <input
                  type='text'
                  value={item}
                  onChange={(e) => onChangePhrase(e, idx)}
            ...
            <div
              span={8}
              className='p-2 mt-2 text-gray-400  text-lg w-1/3'
              key={idx} // Duclair: Do not use Array index in keys
            >
              <div className='border-b-2 border-gray-500 flex'>
                <span className='flex-none'> {`${idx + 1}. `}</span>{' '}
      ...
      {showWarn ? (
        <p className='text-red-500 text-lg mb-2'>
          {t('Please check mnemonic phrases.')}
        </p>
      ) : null}

      <Button size='large' className='access-button  my-8' onClick={verify}>
        {t('Verify')}
      </Button>
    </WalletModal>
  )
