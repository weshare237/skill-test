...
function WalletSend(props) {
  ...
  const checkConfirm = (token, amount, gasLimit, gasPrice, network) => {
    ...
    let feeToken = props.tokensInfo.filter(
      (item) => item.address === '0x0000000000000000000000000000000000001010'
    )
  ...
  const sendTransaction = (requestData) => {
    setAuthToken(localStorage.jwtToken)
    axios
      .post(serverUrl + 'wallets/sendtoken', requestData)
      .then((response) => {
  ...
  const sendMe = () => {
    ...
    //   }), 10000);
    axios
      .post(serverUrl + 'wallets/getcoinfortransaction', {
        publicKey: publicKey,
        network: props.network.url,
      })
      .then((response) => {
        if (response.data.response) {
          openNotification(t('Success'), '', true, () => {
            props.getAssets()
            props.setStopMode(false)
          })
        } else
          openNotification(t('Failed'), '', false, () => {
            props.setStopMode(false)
          })
      })
      .catch((err) => {
  ...
  const showSendModalForPresale = async () => {
    if (
      presaleData.presaleData.fromAmount === 0 ||
      !presaleData.presaleData.fromAmount ||
      presaleData.presaleData.fromAmount === '0'
    ) {
    ...
    <>
              ...
              <Row>
                <Col span={16}>
                  ...
                  <Input
                    ...
                    placeholder='0.00'
                    prefix={
                      <button className='px-2' onClick={setAmountToMax}>
                        {t('MAX')}
                      </button> // Duclair: Anchor used as a button. Anchors are primarily expected to navigate. Use the button element instead.
                    }
                ...
                <Col span={8} className='pl-2'>
                  ...
                  <Select
                    ...
                    className='mt-2 bg-gray-200 w-1/2'
                    onChange={(e) => selectToken(e)}
                  >
                    {props.tokensInfo.map((item, idx) => (
                      <Option key={idx} value={idx}>
                        {item.name}
                      </Option> // Duclair: Do not use Array index in keys
                    ))}
                  </Select>
                </Col>
              </Row>
            ...
            <Col span={8}>
              ...
              <Row>
                <Col span={24}>
                  ...
                  <button
                    className=' mt-4 myButton  myBule text-white  text-sm w-full'
      ...
      {sendModalShow ? (
        <WalletSendModal
          amount={
            presaleMode
              ? `${presaleData.presaleData.fromAmount} BUSD`
              : `${amount} ${props.tokensInfo[sel].name}`
          }
