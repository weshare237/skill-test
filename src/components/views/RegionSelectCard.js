  ...
  const loadMoreData = () => {
    ...
    for (let i = 0; i < names.length; i++) {
      if (names[i].toLowerCase().includes(search.toLowerCase()))
        rawData.push({
          id: i + 1,
          code: countrylist.getCode(names[i]),
          title: names[i],
        })
    }
  ...
  return (
    ...
    >
      <Row className='text-lg font-bold py-4 text-gray-600'>
        <Col span={23}>{t('Country/Area of Residence')}</Col>
        <Col span={1}>
          <button onClick={props.stepDecrement}>
            <CloseOutlined />
          </button>
        </Col>
      </Row>
      <Input
        ...
        className='rounded-lg p-3 search'
      />
        ...
        <List
          ...
          renderItem={(item) => (
            <List.Item key={item.id}>
              <button
                onClick={() => getRegion(item.code)}
                className='grayButton w-full text-lg font-bold text-gray-700 text-left '
              >
                <ReactCountryFlag
