...
function Register() {
  ...
  const increment = (num) => {
    if (step + num >= 2) setStep(2)
    else setStep(step + num)
  }

  const decrement = (num) => {
    if (step - num <= 0) setStep(0)
    else setStep(step - num)
  }
        ...
        >
          ...
          {step == 0 && (
            <RegionCard
              ...
              stepIncrement={() => increment(1)}
              accountStep={() => increment(2)}
              country={region}
            />
          )}
          {step == 1 && (
