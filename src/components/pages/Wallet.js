...
function Wallet() {
  ...
  useEffect(() => {
    ...
    getAssets()
    getTransaction()
    frequent = setInterval(getAssets, 1000 * 60) // Duclair: Promise returned in function argument where a void return was expected.
    if (!publicKey)
      openNotification(
  ...
  const getTokenInfo = async () => {
    ...
    try {
      ...
      // Duclair: Expected a `for-of` loop instead of a `for` loop with this simple iteration.
      for (const element of tokens) {
        let init = { name: '', price: -1, balance: -1, address: '' }
        let { decimal, symbol } = await getTokenBaseInfo(element, network.url)
        init.name = symbol
        init.address = element
        if (symbol.toLowerCase() === 'mgl') init.price = 0.018
        else init.price = await getTokenPriceInUsd(network, element)
              ...
              >
                ...
                <Row className='p-4 '>
                  ...
                  {transations.map((item, idx) => (
                    <Col span={24} key={idx} className='text-overflow'>
                      <a
                        target='_blank'
                    ...
                    // Duclair: Missing "key" prop for element in iterator
                  ))}
                </Row>
              </Col>
            </Row>
